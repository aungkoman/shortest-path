...
L.tileLayer('http://'+localhost_ip+'/google_map/leaflet/tiles/0/{z}/{x}/{y}.png', {
...
function add_marker(lat,long,name){
    // add marker
    L.marker([lat, long], {icon: map_marker_icon}).addTo(mymap).bindPopup(name);
    ...
    .done(function( msg ) {
        ...
        if(returned_data.status == "success"){
            ...
            for(var i =0; i<places.length; i++){
                // add circle to graph
                L.circle([places[i].lat,places[i].long], {radius: 50}).addTo(mymap).bindPopup(places[i].name);
            }
        ...
        if(returned_data.status == "success"){
            ...
            */
            ...
            if(status == "no_route" ){
                ...
                var shortest_path_line = new L.Polyline(pointList, {
                    color: 'blue',
                    weight: 5,
                    opacity: 0.5,
                    smoothFactor: 1
